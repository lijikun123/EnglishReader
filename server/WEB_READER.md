...
- 服务端复用 App 的完整精读提示词，并额外要求只加粗固定表达本身，避免把人名、奖项、地点等可变内容一起标记。默认调用阿里云百炼北京地域的 OpenAI 兼容接口 `https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions` 和 `qwen-plus`；Base URL 与模型都可通过环境变量替换。
- 未配置 Key 时，阅读器会明确显示“AI 尚未配置”和“设置 AI”入口；双语、词组仍会禁用，避免误以为功能已经可用。网页只显示 VPS 配置方法，不接收、保存或转发 API Key。
- 译文和词组按账号、模型、书籍正文版本、章节及段落缓存在当前浏览器的 IndexedDB。刷新和重开后复用缓存；App 现有本地缓存不会上传，因此网页不会直接复用手机上已经生成的结果。
- 网页按当前阅读位置分批处理，优先加载当前段及后续内容，每次最多覆盖附近 7 段；双语与词组请求会错峰发出，不再打开章节后立即处理整章。同一时刻最多发出 2 个上游请求，服务端默认每个账号每分钟最多 60 个 AI 请求；若多标签页等情况偶尔触发限流，网页会等待窗口恢复后自动继续。对百炼 Qwen 请求会关闭不必要的思考模式，以缩短翻译和词组识别的等待；失败结果不缓存。
...
百炼 Key 是能够产生费用的长期凭据，不能填写在网页、保存到浏览器，或发到聊天中。网页的“设置 AI”只会显示上述 VPS 配置位置和命令。
